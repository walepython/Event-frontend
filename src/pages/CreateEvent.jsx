...
const CreateEvent = () => {
  ...
  const fetchVenues = async () => {
    try {
      // You'll need to create this endpoint or fetch venues from events
      const response = await api.get("/venues/");
      setVenues(response.data);
    } catch (error) {
      console.error("Error fetching venues:", error);
      // If no venues endpoint, set empty array
      setVenues([]);
    }
  };
...
const handleSubmit = async (e) => {
    ...
    try {
      // STEP 1: Get current user to add as organizer
      ...
      // Add organizer FIRST (this is required by your backend)
      submitData.append('organizer', currentUser.id);
      ...
      submitData.append('status', 'pending'); // For admin approval
      ...
      const response = await api.post('/eventApi/', submitData, {
        headers: {
      ...
      if (error.response) {
        ...
        if (status === 500) {
          alert('Server Error (500):\n\nPlease check:\n1. You are logged in as organizer/admin\n2. Venue ID is valid (if selected)\n3. All required fields are filled\n\nCheck console for details');
        } else if (status === 400) {
          // Validation errors
          ...
          alert('Permission denied. You must be an organizer or admin to create events.');
        } else {
  ...
  const handleCancel = () => {
    if (window.confirm('Are you sure you want to cancel? All changes will be lost.')) {
      navigate('/organizer-board'); // Fixed path
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <button
            onClick={() => navigate('/organizer-board')} // Fixed path
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  If venue is not listed, you can add venue details in the description
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Leave empty if there's no participant limit
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div className="mt-3 p-3 bg-blue-50 rounded-md">
              <p className="text-sm text-blue-800">
                <strong>Note:</strong> Leave prices as 0 or empty for free events
              </p>
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <p className="mt-1 text-sm text-yellow-700">
                  Your event will be submitted to the admin for approval before it's published. You'll be notified once it's reviewed.
                </p>
              </div>
            </div>
          </div>
